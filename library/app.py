...
<section class="card table-card"><div class="table-head"><div class="summary" id="summary">共 0 条</div></div><div class="table-wrap"><table><thead><tr><th><input class="check" type="checkbox" id="all" onclick="toggleAll(this)"></th><th>排名</th><th>标题</th><th>标签</th><th>查看</th><th>回复</th><th>金币</th><th>作者</th><th>状态</th></tr></thead><tbody id="rows"></tbody></table></div></section>
</main>
<script>
...
async function loadWorks(){const r=await fetch('/api/works?'+params()),d=await r.json();$('summary').textContent=`共 ${d.works.length} 条`;$('rows').innerHTML=d.works.length?d.works.map(w=>{const s=String(w.download_status||'');const cls=s==='已下载'?'done':s.includes('失败')?'fail':s==='有更新'?'update':'';return `<tr><td><input class="check pick" type="checkbox" value="${esc(w.thread_id)}"></td><td>${esc(w.current_rank)}</td><td><a class="title-link" href="${esc(w.url)}" target="_blank" rel="noreferrer">${esc(w.title||'(无标题)')}</a></td><td class="tag-text">${esc((w.tags||[]).join('、'))}</td><td>${esc(w.views)}</td><td>${esc(w.replies)}</td><td>${esc(w.price)}</td><td>${esc(w.author_name)}</td><td><span class="status ${cls}">${esc(s)}</span></td></tr>`}).join(''):'<tr><td colspan="9" class="empty">暂无匹配帖子</td></tr>';$('all').checked=false;}
...
function toggleAll(x){document.querySelectorAll('.pick').forEach(e=>e.checked=x.checked)}function copyUrl(){navigator.clipboard?.writeText($('forum_url').value);$('scanMessage').textContent='版块地址已复制';}function notReady(name){alert(`${name}功能将在后续阶段接入。`)}
loadTags();loadWorks();
</script></body></html>'''
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/api/download":
            ...
            try:
                ...
                count = max(1, min(100, int(data.get("count") or len(rows) or 1)))
                execute = bool(data.get("execute", False))
                settings = DownloadSettings(count=count, max_price=max(0, min(10000, int(data.get("max_price") or 0))), min_balance=max(0, min(1000000, int(data.get("min_balance") or 0))), max_pages_per_work=max(1, min(100, int(data.get("max_pages_per_work") or 6))), minimum_length=max(0, min(100000, int(data.get("minimum_length") or 200))), execute=execute, data_root=DATA_ROOT)
                with LibraryDB(self.db_path) as db:
                    for row in rows:
                        if row.get("download_status") != "已下载" and not row.get("local_path"):
                            row["existing_posts"] = db.list_posts(str(row.get("thread_id") or ""))
                    run_id = db.start_run("auto_download" if mode == "auto" else "download", {"mode": mode, "count": count, "max_price": settings.max_price, "min_balance": settings.min_balance, "max_pages_per_work": settings.max_pages_per_work, "minimum_length": settings.minimum_length, "execute": execute, "filters": {} if mode == "auto" else raw_filters, "thread_ids": [] if mode == "auto" else thread_ids})
