...
def run_download(rows: Iterable[dict[str, Any]], settings: DownloadSettings, *, on_result: Any | None = None) -> list[dict[str, Any]]:
    ...
    def handle_browser_result(result: dict[str, Any]) -> None:
        if result.get("status") != "已抓取":
            saved_result = result
        else:
            metadata = next((row for row in queue if row["thread_id"] == result["thread_id"]), {"thread_id": result["thread_id"]})
            combined: dict[str, dict[str, Any]] = {}
            for post in [*metadata.get("existing_posts", []), *result.get("posts", [])]:
                post_id = str(post.get("remote_post_id") or "")
                if post_id: combined[post_id] = dict(post)
            ...
            saved_result = {**result, "posts": posts, "force_refetch": settings.force, **saved_info}
        saved.append(saved_result)
        if on_result: on_result(saved_result)
    results = run_content_browser(candidates, execute=True, max_price=settings.max_price, min_balance=settings.min_balance, download_limit=settings.count, max_pages_per_work=settings.max_pages_per_work, on_result=handle_browser_result)
    if not saved:
