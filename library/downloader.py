...
def build_download_queue(rows: Iterable[dict[str, Any]], *, count: int | None = None) -> list[dict[str, Any]]:
    queue = [
        row
        for row in rows
        if not SKIP_PURCHASE_TAGS.intersection(row.get("tags", []))
        and row.get("download_status") != "已下载"
        and not row.get("local_path")
    ]
...
def plan_download(rows: Iterable[dict[str, Any]], settings: DownloadSettings) -> list[dict[str, Any]]:
    return [{"thread_id": row["thread_id"], "title": row.get("title", ""), "url": row.get("url", ""), "price": row.get("price", 0), "purchase_status": row.get("purchase_status", "未购买"), "status": "待下载", "execute": settings.execute} for row in build_download_queue(rows, count=settings.count)]
...
def run_download(rows: Iterable[dict[str, Any]], settings: DownloadSettings) -> list[dict[str, Any]]:
    ...
    queue = build_download_queue(rows)
    if not settings.execute:
        return plan_download(queue, settings)
    if not queue:
        return []
    candidates = [{"thread_id": row["thread_id"], "url": row["url"], "title": row.get("title", ""), "price": row.get("price", 0), "purchase_status": row.get("purchase_status", "未购买"), "existing_posts": row.get("existing_posts", [])} for row in queue]
    ...
    for result in results:
        ...
        saved.append({**result, "posts": posts, **saved_info})
    return saved
